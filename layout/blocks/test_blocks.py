...
def spiral_test(grating_coupler_spec,cross_section):
    ...
    for i, spiral in enumerate(spiral_list):
        ...
        spiral_ref = c << spiral
        spiral_ref.connect("o1",gc_ref.ports["o1"], mirror=False if spiral_ref.ports['o1'].y<0 else True)
        gc_end = c << gc
        ...
        [item.move((1000*(i%2),-700*(i//2))) for item in [gc_ref, spiral_ref, gc_end, gc2_ref, gc2_end,straight_ref]]
    return c

@gf.cell
def converter_test(grating_coupler_spec, wg_width=0.43, slab_width=6, taper_length=20, num_taper_pair=2):
    c = gf.Component()
    gc = grating_coupler_spec()
    gc_ref = (c << gc).rotate(180)
    xs_rib = cross_section_with_sleeves(core_width=wg_width, total_width=slab_width, sleeve_layer="SHALLOW_ETCH")
    straight = gf.components.straight(length=10, cross_section=xs_rib)
    straight_ref = c << straight
    straight_ref.connect("o1",gc_ref.ports["o1"])
    prev_straight = straight_ref
    for i in range(num_taper_pair):
        taper1 = c << taper_rib_to_strip(width1=wg_width,width2=wg_width,w_slab1=wg_width,w_slab2=slab_width,length=taper_length)
        taper1.connect("o2",prev_straight.ports["o2"])
        straight_mid_ref =c << straight
        straight_mid_ref.connect("o1",taper1.ports["o1"])
        taper2 = c << taper_rib_to_strip(width1=wg_width,width2=wg_width,w_slab1=wg_width,w_slab2=slab_width,length=taper_length)
...
@gf.cell
def converter_test_array(converter_test_spec, pair_num_list=[20,40,60,80]):
    c = gf.Component()
    for i, pair_num in enumerate(pair_num_list):
        inst = c << converter_test_spec(num_taper_pair=pair_num)
        inst.movey(-120*i)
