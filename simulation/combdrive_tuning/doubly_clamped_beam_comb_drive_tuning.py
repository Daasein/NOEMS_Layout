...
def doubly_clamped_beam_comb_drive_tuning(
    ...
    c = gf.Component()
    spring = spring_5um(spring_length=spring_length,spring_width=spring_width, separation=spring_separation, num_loops=spring_loop_num)
    ...
    shaft = perforated_shaft(width=shaft_width, height=shaft_height,margin=10, mask_offset=20, hole_size=(20,2))

    beam_ref = c << beam
    # create_deep_etch_mask(c, 'bbox', deep_etch_layer='PROTECTION_PL', mask_offset=5)
    ...
    bfs = beam_fixed_support(size=(75,75))
    ...
    finger_spec = partial(combdrive_fingers_5um, finger_length=20, pair_num=finger_pair_num)
    comb_array = combdrive_array(finger_spec=finger_spec, movable_base_width=10, fixed_base_width=20)
    ...
    routing_with_mytaper(
        ...
        cross_section1=metal_wire(core_width=190, wire_width=180, mask_offset=7,deep_etch_layer="DEEP_ETCH_PL"),
    ...
    electrode_ = electrode(width=beam_length-20, height=150, cross_section=xs_metal_wire_wide,metal_offset=10,mask_offset=15)
    ...
    actuation_sensing_pad = pad(size=(beam_length-180,300), metal_offset=10)
    actuation_sensing_pad_n = c.add_ref(actuation_sensing_pad)
    actuation_sensing_pad_n.move(origin=(actuation_sensing_pad_n.xmax,actuation_sensing_pad_n.ymax), destination=(beam.xmax, beam_pad_n.ymax))
    gf.routing.route_single(
        c,
        port1=electrode_n.ports['n1'],
        port2=actuation_sensing_pad_n.ports['S1'].copy(gf.kdb.Trans(x=-(actuation_sensing_pad_n.ports['S1'].center[0]-electrode_n.ports['n1'].center[0])*1000, y=0)),
        cross_section=xs_metal_wire_wide,
        allow_width_mismatch=True,
        )
    actuation_sensing_pad_s = c.add_ref(actuation_sensing_pad)
    actuation_sensing_pad_s.move(origin=(actuation_sensing_pad_s.xmax,actuation_sensing_pad_s.ymin), destination=(beam.xmax, beam_pad_s.ymin))
    gf.routing.route_single(
        c,
        port1=electrode_s.ports['n1'],
        port2=actuation_sensing_pad_s.ports['N1'].copy(gf.kdb.Trans(x=-(actuation_sensing_pad_s.ports['N1'].center[0]-electrode_s.ports['n1'].center[0])*1000, y=0)),
        cross_section=xs_metal_wire_wide,
        allow_width_mismatch=True,
    )
    ...
    return c
c = doubly_clamped_beam_comb_drive_tuning(shaft_height=100, beam_length=1000, finger_pair_num=80, combdrive_array_num_push=5, combdrive_array_num_pull=10, spring_length=150, spring_loop_num=1, combdrive_spacing=100)
c.show()
